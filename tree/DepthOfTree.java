package Leetcode.tree;


public class DepthOfTree {
    
    public int depth(Leetcode.tree.TreeNode root){//递归：求树的深度
        if(root==null)
            return 0;
        int left = depth(root.left);
        int right = depth(root.right);
        return 1+(left>right?left:right);
    }

    public int balanceDepth(Leetcode.tree.TreeNode root){//判断是否为平衡二叉，并返回树的深度，非平衡返回-1
        if(root==null)
            return 0;
        int left = balanceDepth(root.left);
        if(left==-1)//左子树非平衡
            return -1;
        int right = balanceDepth(root.right);
        if(right==-1)//右子树非平衡
            return -1;
        if(left - right <(-1) || left - right > 1)
            return -1;
        return 1+(left>right?left:right);
    }

    public boolean isBalanceTree(TreeNode root){//判断是否为平衡二叉树
        return balanceDepth(root)!=-1;
    }

    public static void main(String[] args) {
        DepthOfTree tree = new DepthOfTree();
        TreeNode node1=new TreeNode(1);
        TreeNode node2=new TreeNode(2);
        TreeNode node3=new TreeNode(3);
        TreeNode node4=new TreeNode(4);
        TreeNode node5=new TreeNode(5);
        TreeNode node6=new TreeNode(6);
        TreeNode node7=new TreeNode(7);
        node1.left= node2;
        node1.right=node3;
        node2.left=node4;
        node2.right=node5;
        node3.left=node6;
        node6.right=node7;
        System.out.println(tree.depth(node1));
        System.out.println(tree.isBalanceTree(node1));

//        StringBuffer s = new StringBuffer();
//        s.append(3);
//        s.append(44);
//
//        for(String ss: new int[]{3,4,5}.toString().split(""))
//            System.out.println(ss);
//        HashMap map = new HashMap();


    }
}

